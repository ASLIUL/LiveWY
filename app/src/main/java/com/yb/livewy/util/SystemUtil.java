...
public class SystemUtil {
    ...
    public static boolean isWifiConnected() {
        ConnectivityManager connectivityManager = (ConnectivityManager) LiveApplication.getInstance().getSystemService(Context.CONNECTIVITY_SERVICE);
...
//        File fileDir = new File(context.getPackageName()+"/");
//        if (!fileDir.exists()){
//            fileDir.mkdirs();
//        }
...
//            if (isCompress) {
//                ToastUtil.showToast("保存妹纸成功n(*≧▽≦*)n");
//            } else {
//                ToastUtil.showToast("保存妹纸失败ヽ(≧Д≦)ノ");
//            }
//            fos.flush();
//            fos.close();
//        } catch (IOException e) {
...
//    }
    ...
    public static int dp2px(float dpValue) {
        final float scale = LiveApplication.getInstance().getResources().getDisplayMetrics().density;
        return (int) (dpValue * scale + 0.5f);
    }
    ...
    public static int getScreenHeight(Context context){
        ...
        Point point = new Point();
        manager.getDefaultDisplay().getSize(point);
        Log.d("TAG", "getScreenWidth: "+point.y);
        return point.y;
    }
}
